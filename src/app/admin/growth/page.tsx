                ...
                <Card className="col-span-4 flex flex-col shadow-md border-slate-200 overflow-hidden">
                    <CardHeader className="border-b bg-white">
                        <CardTitle className="text-lg flex items-center gap-2">
                            <TrendingUp className="text-[#be123c] w-5 h-5" /> High Propensity Leads
                        </CardTitle>
                    </CardHeader>
                            ...
                            >
                                <div className="flex justify-between items-start mb-2">
                                    ...
                                    <span className="bg-[#be123c]/10 text-[#be123c] text-[10px] font-bold px-2 py-0.5 rounded-full uppercase">
                                        {lead.buyLikelihood}% Match
                                    </span>
                                </div>
                                <div className="text-[11px] text-slate-500 font-medium whitespace-nowrap overflow-hidden text-ellipsis">Current: {lead.currentProduct}</div>
                                <div className="mt-3 pt-3 border-t border-slate-50 flex justify-between items-center">
                                    <div>
                                        <div className="text-[10px] font-bold text-slate-400 uppercase mb-0.5 tracking-tighter">Opportunity</div>
                ...
                <Card className="col-span-8 shadow-md border-slate-200 flex flex-col relative overflow-hidden">
                    {selectedLead ? (
                        <CardContent className="p-0 flex flex-col h-full overflow-y-auto">
                            ...
                            <div className="p-8 flex-1 space-y-6">
                                <h3 className="font-bold text-[#be123c] flex items-center gap-2 text-[11px] tracking-[0.2em] uppercase">
                                    <Sparkles className="w-4 h-4" /> Personalized Dynamic Pitch
                                </h3>
                                ...
                                <div className="bg-slate-50 border-2 border-slate-100 p-8 rounded-[2rem] text-xl leading-relaxed text-slate-700 shadow-inner relative italic">
                                    <div className="absolute -top-3 left-8 bg-white px-3 py-1 rounded-full border text-[9px] font-bold text-slate-400 uppercase tracking-widest">Rommaana Voice Processing Mesh</div>
                                    {isGenerating ? (
                                        <div className="flex items-center gap-3 text-slate-400">
                                            <div className="w-2 h-2 rounded-full bg-[#be123c] animate-ping"></div>
                                            Analyzing customer behavior & historical affinity...
                                        </div>
                                    ) : (
                                        `"${script}"`
                                    )}
                                ...
                                {/* Objection Handlers */}
                                <div className="mt-8">
                                    <p className="text-[10px] font-bold text-slate-400 uppercase mb-4 tracking-[0.15em] border-l-2 border-slate-200 pl-2">Sentiment Pivot Options</p>
                            ...
                            <div className="p-6 bg-slate-50/50 border-t flex items-center gap-6 text-slate-400 mt-auto">
                                <div className="flex items-center gap-2 text-[#be123c] font-bold text-[10px] uppercase tracking-tighter">
                                    <Mic className="w-4 h-4 animate-pulse" /> Telephony Online
                                </div>
                                <div className="h-1.5 flex-1 bg-slate-200 rounded-full overflow-hidden">
                                    <div className="h-full w-2/3 bg-emerald-500 rounded-full"></div>
                                </div>
                                <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded border border-emerald-100">Live Vocal Sentiment: Enthusiastic (92%)</span>
                            </div>
                        ...
                        <div className="h-full flex items-center justify-center text-slate-400 flex-col bg-slate-50/50">
                            ...
                            <h3 className="font-bold text-slate-600 mb-1">Select A Lead</h3>
                            <p className="text-xs text-slate-400 font-medium uppercase tracking-widest">Sales Agentic Mesh standby</p>
                        </div>
                    )}
                </Card>
            </main>
